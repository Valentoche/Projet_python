...
def Labyrinthe(noms_joueurs, couleurs_joueurs, humain=False, nb_tours=100, duree_objet=10):
    ...
    return {"Labyrinthe":plateau, 'participants':participants, 'nb_tour': nb_tours, 'duree_objet':duree_objet, 'rangee_interdite':None, 'direction_interdite':None}
...
def get_nb_tours_restants(labyrinthe):
    ...
    """
    return labyrinthe['nb_tour'] - labyrinthe['participants']["Current"]
...
def get_coordonnees_joueur_courant(labyrinthe):
    ...
    return get_coordonnees_joueur(labyrinthe['Labyrinthe'][0], joueur_courant)
...
def prendre_joueur_courant(labyrinthe, lig, col):
    ...
    """
    joueur_courant = get_joueur_courant(labyrinthe['participants'])
    carte = get_valeur(labyrinthe['Labyrinthe'][0], lig, col)
...
def get_carte_a_jouer(labyrinthe):
    ...
    """
    return labyrinthe['Labyrinthe'][1]
...
def jouer_carte(labyrinthe, direction, rangee):
    ...
    if coup_interdit(labyrinthe, direction, rangee) == False :
        ...
        if direction == 'N' :
            carte_enlevee = decalage_colonne_en_bas(labyrinthe['Labyrinthe'][0], rangee, labyrinthe['Labyrinthe'][1])
            lave_et_transfert_joueurs(carte_enlevee, get_valeur(labyrinthe['Labyrinthe'][0], 0, rangee))
            labyrinthe['Labyrinthe'][1] = carte_enlevee
            return "Carte insérée !"

        if direction == 'S' :
            carte_enlevee = decalage_colonne_en_haut(labyrinthe['Labyrinthe'][0], rangee, labyrinthe['Labyrinthe'][1])
            lave_et_transfert_joueurs(carte_enlevee, get_valeur(labyrinthe['Labyrinthe'][0], get_nb_lignes(labyrinthe['Labyrinthe'][0]), rangee))
            labyrinthe['Labyrinthe'][1] = carte_enlevee
            return "Carte insérée !"
        
        if direction == 'E' :
            carte_enlevee = decalage_ligne_a_droite(labyrinthe['Labyrinthe'][0], rangee, labyrinthe['Labyrinthe'][1])
            lave_et_transfert_joueurs(carte_enlevee, get_valeur(labyrinthe['Labyrinthe'][0], rangee, 0))
            labyrinthe['Labyrinthe'][1] = carte_enlevee
            return "Carte insérée !"

        if direction == 'O' :
            carte_enlevee = decalage_ligne_a_gauche(labyrinthe['Labyrinthe'][0], rangee, labyrinthe['Labyrinthe'][1])
            lave_et_transfert_joueurs(carte_enlevee, get_valeur(labyrinthe['Labyrinthe'][0], rangee, get_nb_colonnes(labyrinthe['Labyrinthe'][0])))
            labyrinthe['Labyrinthe'][1] = carte_enlevee
            return "Carte insérée !"
...
def tourner_carte(labyrinthe, sens='H'):
    ...
    """
    if sens == 'H' :
        tourner_horaire(labyrinthe['Labyrinthe'][1])
        return "Carte tournée!"
    elif sens == 'A':
        tourner_antihoraire(labyrinthe['Labyrinthe'][1])
        return "Carte tournée!"
    else :
...
def peindre(labyrinthe, direction):
    ...
    print(coordonnee_joueur)
    joueur_courant = get_joueur_courant(labyrinthe['participants'])
    liste_joueur_touchees = list()

    if get_objet_joueur(joueur_courant) == 2 :
        peindre = peindre_direction_couleur(labyrinthe['Labyrinthe'][0], coordonnee_joueur[0], coordonnee_joueur[1], direction, get_couleur_joueur(joueur_courant), get_reserve_peinture(joueur_courant), True, False)
        ...
        for dire in sens :
            peindre = peindre_direction_couleur(labyrinthe['Labyrinthe'][0], coordonnee_joueur[0], coordonnee_joueur[1], dire, get_couleur_joueur(joueur_courant), get_reserve_peinture(joueur_courant), False, False)
            liste_joueur_touchees.append(peindre[0])
            ajouter_peinture(joueur_courant, -peindre[1])

    else :
        peindre = peindre_direction_couleur(labyrinthe['Labyrinthe'][0], coordonnee_joueur[0], coordonnee_joueur[1], direction, get_couleur_joueur(joueur_courant), get_reserve_peinture(joueur_courant), False, False)
        liste_joueur_touchees.append(peindre[0])
        ajouter_peinture(joueur_courant, -peindre[1])
...
def deplacer(labyrinthe, direction):
    ...
    * mettre à jour la réserve de peinture du joueur en fonction de la couleur de la case d'arrivée
    ...
    passage = passage_plateau(labyrinthe['Labyrinthe'][0], position[0], position[1], direction)

    if passage != None :
        prendre_joueur_courant(labyrinthe, position[0], position[1])
        poser_joueur_courant(labyrinthe, passage[0], passage[1])

        if get_couleur(get_valeur(labyrinthe['Labyrinthe'][0], passage[0], passage[1])) == get_couleur_joueur(joueur):
            ajouter_peinture(joueur, POINTS["couleur_joueur"])
        elif get_couleur(get_valeur(labyrinthe['Labyrinthe'][0], passage[0], passage[1])) == None :
            ajouter_peinture(joueur, POINTS["couleur_neutre"])
        else :
            ajouter_peinture(joueur, POINTS["couleur_adversaire"])
        
        carte = get_valeur(labyrinthe['Labirinthe'][0], passage[0], passage[1])
...
def interpreter_ordre(labyrinthe, ordre):
    ...
    if len(ordre) == 4 :
        ...
        peinture = peindre(labyrinthe, ordre[1])
        if ordre[2] == 'T' :
            tourner = tourner_carte(labyrinthe, ordre[3])
            inseree = jouer_carte(labyrinthe, ordre[4], ordre[5])
            return (peinture +'\n'+ tourner +'\n'+ inseree)
...
def finir_tour(labyrinthe):
    ...
    couleur = nb_cartes_par_couleur(labyrinthe['Labyrinthe'][0])
